...
```json
{"expenses": [{"description": "項目名", "amount": 金額（数値）, "category": "カテゴリ名", "date": "YYYY-MM-DD or null"}]}
```
- JSON以外にも、自然な日本語での確認や補足コメントを付けてください
- dateフィールド: ユーザーが日付を言及した場合は "YYYY-MM-DD" 形式で、言及がない場合は null を設定
- 「昨日」「3月4日」「先週の火曜」などの相対的な日付表現も適切に解釈してください
- 金額が不明確な場合は確認してください
...
```json
{"edits": [{"original_description": "元の項目名（部分一致可）", "description": "新しい項目名", "amount": 新しい金額, "category": "新しいカテゴリ", "date": "YYYY-MM-DD or null"}]}
```
